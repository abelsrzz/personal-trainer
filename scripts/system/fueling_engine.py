...
def supplement_ml_for_amount(item: dict[str, Any], amount_g: float | None = None) -> float | None:
    reference_weight_g = safe_float(item.get("reference_weight_g"))
    reference_volume_ml = safe_float(item.get("reference_volume_ml"))
    target_amount_g = amount_g if amount_g is not None else safe_float(item.get("serving_size"))
    if not reference_weight_g or not reference_volume_ml or not target_amount_g:
        return None
    if reference_weight_g <= 0 or reference_volume_ml <= 0 or target_amount_g <= 0:
        return None
    return target_amount_g * reference_volume_ml / reference_weight_g


def supplement_reference_label(item: dict[str, Any]) -> str | None:
    reference_weight_g = safe_float(item.get("reference_weight_g"))
    reference_volume_ml = safe_float(item.get("reference_volume_ml"))
    if not reference_weight_g or not reference_volume_ml:
        return None
    weight_text = f"{reference_weight_g:g} g"
    volume_text = f"{reference_volume_ml:g} ml"
    return f"{volume_text} = {weight_text}"


def supplement_serving_label(item: dict[str, Any]) -> str:
    serving_size = item.get("serving_size")
    serving_unit = item.get("serving_unit")
    ml_for_serving = supplement_ml_for_amount(item)
    if ml_for_serving is None:
        return f"{serving_size} {serving_unit}"
    return f"{serving_size} {serving_unit} (~{ml_for_serving:.1f} ml)"
...
def supplements_catalog() -> list[dict[str, Any]]:
    ...
    for item in items:
        if not isinstance(item, dict) or not item.get("enabled"):
            continue
        enriched = dict(item)
        reference_label = supplement_reference_label(enriched)
        if reference_label:
            enriched["reference_label"] = reference_label
        serving_volume_ml = supplement_ml_for_amount(enriched)
        if serving_volume_ml is not None:
            enriched["serving_volume_ml"] = round(serving_volume_ml, 1)
...
def render_fueling_markdown(payload: dict[str, Any]) -> str:
    ...
    for item in payload.get("supplements", []):
        serving_label = supplement_serving_label(item)
        reference_label = item.get("reference_label")
        if reference_label:
            lines.append(f"- `{item.get('name')}` ({item.get('category')}): {serving_label} · referencia `{reference_label}`")
        else:
            lines.append(f"- `{item.get('name')}` ({item.get('category')}): {serving_label}")
    lines.extend(["", "## Races", ""])
