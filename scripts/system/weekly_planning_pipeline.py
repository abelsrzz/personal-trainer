...
def build_range_prompt(target_start: date, target_end: date, premise: str, *, mode: str) -> str:
    ...
    return (
        ...
        "MODO EJECUCION: no interactivo. No hagas plan largo, no uses todolist y no expliques intenciones. "
        "Lee solo lo imprescindible y escribe los archivos reales en este turno.\n\n"
        ...
        "1. Lee estos archivos y nada mas salvo necesidad clara: athlete/status_dashboard.md, planning/coach_decision.md, system/state/athlete_state.json, planning/weeks/semana_actual.md, los markdown semanales listados arriba, training/planned/workouts/library_run_templates.yaml y YAMLs existentes del rango.\n"
        "2. Modifica solo la fuente canonica: markdown semanal afectado y YAMLs fechados en training/planned/workouts/ dentro del rango. No uses overlays temporales.\n"
        ...
        f"5. {extra_rules}\n"
        "6. Si retiras sesiones del rango, elimina tambien los YAMLs y deja el markdown semanal consistente.\n"
        "7. Verifica con `git diff --name-only` que hay cambios en el markdown/YAMLs del rango. No hagas commit.\n"
    )
