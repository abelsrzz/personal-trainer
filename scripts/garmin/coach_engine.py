...
def evaluate_goal_gates(activities: list[dict[str, Any]], reviews: list[dict[str, Any]], feedback_items: list[dict[str, Any]], shin_entries: list[dict[str, Any]], as_of: date) -> dict[str, Any]:
    ...
    checkpoint_rules = config.get("block_checkpoints", []) if isinstance(config.get("block_checkpoints"), list) else []
    checkpoints = []
    for item in checkpoint_rules:
        if not isinstance(item, dict):
            continue
        threshold_km = float(item.get("min_avg_weekly_km") or 0.0)
        max_pain = item.get("max_shin_pain")
        pace_requirement = item.get("requires_5k_s")
        passed = avg_weekly_km >= threshold_km
        if max_pain is not None:
            passed = passed and (shin_pain is None or shin_pain <= float(max_pain))
        if pace_requirement is not None:
            passed = passed and best_5k is not None and best_5k <= float(pace_requirement)
        checkpoints.append(
            {
                "name": item.get("name") or "Checkpoint",
                "phase": item.get("phase") or "general",
                "passed": passed,
                "evidence": f"Volumen {avg_weekly_km:.1f} km/sem, periostio {shin_pain if shin_pain is not None else '-'}, 5k {seconds_to_time(best_5k)}.",
            }
        )
...
def build_decision(activities: list[dict[str, Any]], reviews: list[dict[str, Any]], feedback_items: list[dict[str, Any]], shin_entries: list[dict[str, Any]], as_of: date) -> dict[str, Any]:
    ...
    progression = progression_window(reviews, shin_entries, as_of=as_of, coach_status=status)
    replanning = recommend_replan(coach_status=status, shin_band=str(progression.get("shin_status", {}).get("band") or "green"), risky_review=bool(risky_review), latest_pain=shin_pain)
...
def render_dashboard(payload: dict[str, Any]) -> str:
    ...
    paces = payload.get("training_paces", {})
    ...
    lines.extend(
        [
            ...
            "",
            f"- Estado: `{progression.get('status') or '-'}`",
            f"- Ultima semana absorbida: `{(progression.get('last_absorbed_week') or {}).get('week') or '-'}`",
            f"- Running base absorbido: `{fmt_float(progression.get('baseline_running_km'))} km`",
            f"- Siguiente objetivo por default: `{fmt_float((progression.get('next_running_target_range_km') or {}).get('min'))}-{fmt_float((progression.get('next_running_target_range_km') or {}).get('max'))} km`",
            f"- Bloqueos: `{', '.join(progression.get('blocked_dimensions') or []) or '-'}`",
            f"- Mantener bici de soporte: `{ 'si' if progression.get('keep_bike_support_session') else 'no'}`",
            "",
            "## Objetivo 35:00",
            "",
    ...
    if gates.get("block_checkpoints"):
        lines.extend(["", "## Checkpoints De Bloque", ""])
        for item in gates["block_checkpoints"]:
            marker = "OK" if item.get("passed") else "NO"
            lines.append(f"- `{marker}` {item.get('name')}: {item.get('evidence')}")
    if paces:
        lines.extend(["", "## Bandas Actuales", ""])
        for key in ["fartlek", "tempo", "ten_k_specific", "vo2"]:
            label = key.replace("_", " ")
            band = ((paces.get("labels") or {}).get(key) or {})
            lines.append(f"- `{label}`: `{band.get('min', '-')}` a `{band.get('max', '-')}`")
...
def render_decision(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            f"- Ultima semana absorbida: `{(progression.get('last_absorbed_week') or {}).get('week') or '-'}`",
            f"- Objetivo de running por default: `{fmt_float((progression.get('next_running_target_range_km') or {}).get('min'))}-{fmt_float((progression.get('next_running_target_range_km') or {}).get('max'))} km`",
            f"- Sesion de bici de soporte: `{ 'si' if progression.get('keep_bike_support_session') else 'no'}`",
            f"- Frecuencia fartlek: `{progression.get('fartlek_frequency') or '-'}`",
        ]
    )
    return "\n".join(lines)
...
def build_payload(as_of: date, days: int) -> dict[str, Any]:
    ...
    context = active_context(as_of)
    latest_feedback_item = latest_feedback(feedback_items, as_of)
    zones = load_yaml(ROOT / "athlete" / "zones.yaml").get("zones", {})
    return {
        ...
        "running_tolerance": load_running_tolerance(),
        "training_paces": build_training_paces(activities, zones, as_of=as_of),
        "hybrid_training": {
            "weeks": weekly_training_mix(reviews, until=as_of)[-8:],
        },
        "feedback_summary": {
