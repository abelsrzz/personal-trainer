...
- `system/state/athlete_state.json`
...
- use `system/state/athlete_state.json` to read the last absorbed week, hybrid load, impact-return stage, pace bands and replanning hints before changing load
- default post-return running progression is roughly `+5%` over the last absorbed running week only when state remains green enough to allow it
- preserve bike support as an active planning tool, not only a rehabilitation placeholder
