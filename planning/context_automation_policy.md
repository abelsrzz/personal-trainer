...
- `system/capabilities/registry.yaml`
...
- `system/state/athlete_state.json`: mandatory whenever hybrid load, impact return, pace-band progression, replanning hints or blocked progression dimensions matter.
- `system/capabilities/registry.yaml`: mandatory whenever the task may use a capability-backed refresh path instead of raw file reads.
...
- For powder supplements, interpret `serving_size` as the target amount in grams and use the stored `reference_volume_ml`/`reference_weight_g` conversion to express approximate `ml` when giving instructions.
...
- If `system/state/athlete_state.json` exists, prefer it as the first consolidated read for athlete state, but verify the underlying source files when a change requires deeper reasoning or write-back.
- If a capability exists for the needed signal, prefer refreshing/reading it through the capability workflow before manually recomputing stale local context.
