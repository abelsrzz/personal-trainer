...
- after impact return starts, default to roughly `+5%` running progression over the last absorbed week only when the state and coach color allow it
- keep at least one bike support session in most weeks while running durability is still being rebuilt
- use fartlek regularly in early return/base phases as the first flexible quality bridge before denser interval work
- use the pace bands already derived in `athlete_state.json` instead of freezing old paces manually
...
3. detect any need to replan the current week from `planning/coach_decision.md` and `system/state/athlete_state.json`
